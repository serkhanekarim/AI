...
def main(args, project_name):
    ...
    id_audio_to_keep = [index for index in tqdm(range(len(list_audio_path_preprocessing))) if duration_minimum_recording <= librosa.get_duration(filename=list_audio_path_preprocessing[index]) <= 10000000]
    list_subtitle = [list_subtitle[index] for index in id_audio_to_keep]
    list_audio_path_preprocessing = [list_audio_path_preprocessing[index] for index in id_audio_to_keep]
