...
class MediaScraper:
    ...
    def get_audio_youtube_data(self, url, audio_format, subtitle_language, directory_output):
        ...
        filename_audio = os.path.join(directory_output,youtube_title + "-" + youtube_id + "." + audio_format).replace("|","_")
        ...
        if os.path.isfile(filename_audio):
            ...
            command = "youtube-dl " + url + " --write-sub --sub-lang " + subtitle_language + " --extract-audio --audio-format " + audio_format + " --audio-quality 0 --output " + "'" + os.path.join(directory_output,"%(title)s-%(id)s.%(ext)s") + "' --skip-download"
        else:
